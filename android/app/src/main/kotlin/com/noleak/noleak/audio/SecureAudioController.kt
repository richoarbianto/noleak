 ...
 * 
 * Uses MediaPlayer with custom MediaDataSource for playback.
 * Audio data is streamed by chunk when available to reduce RAM usage.
 * 
 * SECURITY:
 * - Audio data is decrypted from vault on open or per-chunk
...
class SecureAudioController(
    ...
    suspend fun open(fileId: ByteArray): Boolean = withContext(Dispatchers.IO) {
        try {
            val entry = vaultBridge.listFiles().getOrNull()
                ?.firstOrNull { it.fileId.contentEquals(fileId) }

            if (entry != null && entry.chunkCount > 0 && entry.size > 0) {
                dataSource = VaultChunkedAudioDataSource(
