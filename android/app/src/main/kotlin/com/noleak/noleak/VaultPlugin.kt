...
class VaultPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware,
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "getKdfInfo" -> handleGetKdfInfo(result)
            "getVaultKdfParams" -> handleGetVaultKdfParams(result)
            // Multi-vault methods
            ...
            vaultBridge.readTextPreview(fileId, maxBytes).fold(
                onSuccess = { preview ->
                    try {
                        ...
                        VaultEngine.secureZeroize(preview.data)
                    }
                },
                onFailure = { e -> result.error("READ_FAILED", e.message, null) }
            )
    ...
    private suspend fun copyVaultToUri(uri: Uri): Boolean {
        ...
        suspend fun emitProgress(bytes: Long, total: Long, complete: Boolean = false, err: String? = null) {
            withContext(Dispatchers.Main) {
                emitTransferProgress("export_vault", bytes, total, complete, err)
            }
        }
        ...
        return try {
            ...
            if (success) {
                emitProgress(totalBytes, totalBytes, complete = true)
            }
            success
        } catch (e: Exception) {
            emitProgress(bytesCopied, totalBytes, err = "Export failed")
            false
        }
    }
    ...
    private fun handleGetKdfInfo(result: MethodChannel.Result) {
        val info = vaultBridge.getKdfInfo()
        if (info == null) {
            result.error("KDF_ERROR", "Failed to get KDF info", null)
            return
        }
        result.success(info)
    }

    private fun handleGetVaultKdfParams(result: MethodChannel.Result) {
        if (!vaultBridge.isVaultOpen()) {
            result.error("VAULT_LOCKED", "Vault must be open to get KDF params", null)
            return
        }
        val params = vaultBridge.getVaultKdfParams()
        if (params == null) {
            result.error("KDF_ERROR", "Failed to get vault KDF params", null)
            return
        }
