...
class VaultEngine private constructor(private val context: Context) {
    ...
    private external fun nativeGetKdfInfo(): IntArray?
    private external fun nativeGetVaultKdfParams(): IntArray?
     ...
     * Returns map with: memoryMb, iterations, parallelism, profile
     */
    fun getKdfInfo(): Map<String, Any>? {
        val params = nativeGetKdfInfo() ?: return null
        ...
        return mapOf(
            "memoryMb" to memMb,
            "iterations" to iter,
            "parallelism" to params[2],
            "profile" to profile,
            "deviceRamMb" to getTotalRamMb()
        )
    }
     ...
     * Returns map with: memoryMb, iterations, parallelism, profile
     */
    fun getVaultKdfParams(): Map<String, Any>? {
        if (!nativeIsOpen()) return null
        
        val params = nativeGetVaultKdfParams() ?: return null
        if (params.size < 3) return null
