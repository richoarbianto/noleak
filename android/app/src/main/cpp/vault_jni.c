...
// Helper: Zeroize Java byte array contents
static void jbytearray_zeroize(JNIEnv* env, jbyteArray array) {
    if (!array) return;
    jsize len = (*env)->GetArrayLength(env, array);
    if (len <= 0) return;
...
Java_com_noleak_noleak_vault_VaultEngine_nativeChangePassword(
    ...
    if (!old_pass || !new_pass) {
        jbytearray_zeroize(env, oldPassphrase);
        jbytearray_zeroize(env, newPassphrase);
        if (old_pass) {
