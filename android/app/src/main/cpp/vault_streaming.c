...
static int save_state(const streaming_import_state_t *state) {
  ...
  // SECURITY: Do not persist filename or MIME metadata in plaintext.
  len = 0;
  if (write(fd, &len, sizeof(len)) != sizeof(len)) {
    close(fd);
    return STREAMING_ERR_IO;
...
static int load_state(const uint8_t import_id[VAULT_ID_LEN],
  ...
  if (!state->file_name) {
    state->file_name = strdup("imported");
  }
  if (!state->mime_type) {
    state->mime_type = strdup("");
  }
...
#ifndef NDEBUG
  uint64_t completed_size = state->file_size;
#endif
  streaming_abort(import_id);

  LOGI("streaming_finish: SUCCESS (%llu bytes)",
       (unsigned long long)completed_size);
...
void streaming_free_state(streaming_import_state_t *state) {
  if (!state)
    return;
  if (state->source_uri) {
    vault_zeroize(state->source_uri, strlen(state->source_uri));
    free(state->source_uri);
    state->source_uri = NULL;
  }
  if (state->file_name) {
    vault_zeroize(state->file_name, strlen(state->file_name));
    free(state->file_name);
    state->file_name = NULL;
  }
  if (state->mime_type) {
    vault_zeroize(state->mime_type, strlen(state->mime_type));
