...
#ifndef VAULT_DEBUG_LOGS
#define VAULT_DEBUG_LOGS 0
#endif
...
typedef struct {
  ...
  // Header tracking for journaled header format
  size_t header_size;
  uint32_t header_seq;
  uint32_t header_slot_size;
  uint32_t header_slot_count;
  int header_is_journal;

  // Index padding/capacity tracking for in-place updates
