  ...
  Future<void> _openVideo() async {
    try {
      ...
      final chunkCount = widget.entry.chunkCount > 0
          ...
          : ((widget.entry.size + (1024 * 1024) - 1) ~/
              (1024 * 1024)); // Ceiling division

      SecureLogger.d('VideoPlayer', 'Opening video "${widget.entry.name}"');
      SecureLogger.d(
          'VideoPlayer', 'size=${widget.entry.size}, chunkCount=$chunkCount');
      ...
      SecureLogger.d('VideoPlayer',
          'handle=${result['handle']}, textureId=${result['textureId']}');
      SecureLogger.d('VideoPlayer',
          '${result['width']}x${result['height']}, duration=${result['durationMs']}ms');
        ...
        children: [
          ...
          SliderTheme(
            ...
            child: Slider(
              value: _positionMs
                  .toDouble()
                  .clamp(0, _durationMs > 0 ? _durationMs.toDouble() : 1),
              min: 0,
