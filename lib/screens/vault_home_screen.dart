...
class _VaultHomeScreenState extends State<VaultHomeScreen> {
  ...
  void initState() {
    ...
    _importProgressSub =
        _streamingImportService.progressStream.listen((progress) {
      ...
      if (_activeImportId != null &&
          !listEquals(progress.importId, _activeImportId)) return;
      ...
      if (mounted) {
        setState(() {
          ...
          _importProgress =
              progress.isComplete ? 1.0 : (isFinalizing ? 0.99 : normalized);
        });
      }
    });
  ...
  Future<void> _importFile() async {
    ...
    try {
      ...
      // Keep the vault/session alive while Android hands control to the SAF picker.
  ...
  Future<void> _importFolder() async {
    SecureLogger.d('VaultHomeScreen',
        '_importFolder: starting, currentFolderPath=$_currentFolderPath');
    final sessionId = DateTime.now().microsecondsSinceEpoch;
    ...
    try {
      ...
      if (folderInfo == null) {
        SecureLogger.d(
            'VaultHomeScreen', '_importFolder: user cancelled folder picker');
        return;
      }
      SecureLogger.d('VaultHomeScreen',
          '_importFolder: folder picked, uri=${folderInfo['uri']}');
      ...
      SecureLogger.d('VaultHomeScreen',
          '_importFolder: calling VaultChannel.importFolder');
      ...
      SecureLogger.d('VaultHomeScreen',
          '_importFolder: received ${files.length} files, skipped=$skipped');
      ...
      for (final item in files) {
        ...
        SecureLogger.d('VaultHomeScreen',
            '_importFolder: file folder from native=$folder');
        if (_currentFolderPath.isNotEmpty) {
        ...
        SecureLogger.d(
            'VaultHomeScreen', '_importFolder: final folder=$folder');
        imported[fileId] = folder;
      }

      if (imported.isNotEmpty) {
        SecureLogger.d('VaultHomeScreen',
            '_importFolder: calling applyImportedFiles with ${imported.length} files');
        await widget.stateManager.applyImportedFiles(imported);
        SecureLogger.d(
            'VaultHomeScreen', '_importFolder: calling refreshEntries');
        ...
        await widget.stateManager.refreshEntriesOnly();
        SecureLogger.d('VaultHomeScreen',
            '_importFolder: folders after import=${widget.stateManager.folders}');
      }
      ...
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            ListTile(
              leading:
                  const Icon(Icons.folder, color: CyberpunkTheme.neonGreen),
  ...
  Future<void> _createFolder() async {
    SecureLogger.d('VaultHomeScreen',
        '_createFolder: starting, currentFolderPath=$_currentFolderPath');
    String? name;
    ...
    if (trimmed.isEmpty) {
      SecureLogger.d(
          'VaultHomeScreen', '_createFolder: name is empty after trim');
      return;
    }
    SecureLogger.d('VaultHomeScreen',
        '_createFolder: creating folder "$trimmed" in parent "$_currentFolderPath"');
    setState(() => _isVaultOp = true);
    // Freeze timers during folder creation (involves vault write operation)
    widget.stateManager.freezeTimers();
    try {
      await widget.stateManager
          .createFolder(trimmed, parent: _currentFolderPath);
      SecureLogger.d(
          'VaultHomeScreen', '_createFolder: folder created successfully');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(
          ...
          style: const TextStyle(
              color: CyberpunkTheme.neonGreen, letterSpacing: 2),
        ),
        content: SizedBox(
          ...
          child: ListView.separated(
            ...
            separatorBuilder: (_, __) =>
                const Divider(height: 1, color: CyberpunkTheme.surfaceBorder),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Moved to ${destination.isEmpty ? 'root' : destination}'),
            backgroundColor: CyberpunkTheme.neonGreen.withOpacity(0.9),
          ),
        );
      }
  ...
  Future<void> _copyFile(VaultEntry entry) async {
    SecureLogger.d(
        'VaultHomeScreen', '_copyFile: starting copy of ${entry.name}');
    final destination = await _selectFolder(title: 'COPY TO FOLDER');
    if (destination == null) {
      SecureLogger.d(
          'VaultHomeScreen', '_copyFile: user cancelled folder selection');
      return;
    }
    ...
    try {
      SecureLogger.d(
          'VaultHomeScreen', '_copyFile: calling VaultChannel.copyFile');
      final newFileId = await VaultChannel.copyFile(entry.fileId);
      SecureLogger.d('VaultHomeScreen',
          '_copyFile: newFileId isEmpty=${newFileId.isEmpty}');
      if (newFileId.isNotEmpty) {
        SecureLogger.d(
            'VaultHomeScreen', '_copyFile: calling applyImportedFiles');
        await widget.stateManager.applyImportedFiles({newFileId: destination});
        SecureLogger.d(
            'VaultHomeScreen', '_copyFile: calling refreshEntriesOnly');
        await widget.stateManager.refreshEntriesOnly();
        SecureLogger.d(
            'VaultHomeScreen', '_copyFile: copy completed successfully');
      }
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Copied to ${destination.isEmpty ? 'root' : destination}'),
            backgroundColor: CyberpunkTheme.neonGreen.withOpacity(0.9),
          ),
        );
      }
  ...
  Future<void> _showExportDialog() async {
    ...
    try {
      ...
      final ok = await widget.stateManager.exportVault();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content:
              Text(ok ? 'Vault exported successfully' : 'Export cancelled'),
          backgroundColor: ok
  ...
  Future<void> _showChangePasswordDialog() async {
    ...
    if (currentPassword.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Current password required'),
              backgroundColor: CyberpunkTheme.error),
        );
      }
      return;
    ...
    try {
      final success =
          await VaultChannel.changePassword(currentPassword, newPassword);
  ...
  Future<void> _deleteFile(VaultEntry entry) async {
    SecureLogger.d(
        'VaultHomeScreen', '_deleteFile: starting deletion of ${entry.name}');
    ...
    if (confirm == true) {
      ...
      try {
        SecureLogger.d(
            'VaultHomeScreen', '_deleteFile: calling VaultChannel.deleteFile');
        await VaultChannel.deleteFile(entry.fileId);
        SecureLogger.d(
            'VaultHomeScreen', '_deleteFile: calling removeFileFromFolderMap');
        await widget.stateManager.removeFileFromFolderMap(entry.fileId);
        SecureLogger.d(
            'VaultHomeScreen', '_deleteFile: calling refreshEntriesOnly');
        await widget.stateManager.refreshEntriesOnly();
        SecureLogger.d(
            'VaultHomeScreen', '_deleteFile: deletion completed successfully');
  ...
  Future<void> _exportFile(VaultEntry entry) async {
    SecureLogger.d(
        'VaultHomeScreen', '_exportFile: starting export of ${entry.name}');
    widget.stateManager.recordActivity();
      ...
      builder: (context) => AlertDialog(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: CyberpunkTheme.warning.withOpacity(0.3)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
                    children: [
                      Icon(Icons.shield_outlined,
                          color: CyberpunkTheme.warning, size: 18),
                      const SizedBox(width: 8),
                      const Text(
                        'Security Notice',
                        style: TextStyle(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            style:
                TextButton.styleFrom(foregroundColor: CyberpunkTheme.warning),
            child: const Text('EXPORT'),
          ),
        ],
      ),
    ...
    try {
      final success = await VaultChannel.exportFile(entry.fileId, entry.name);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text(success ? 'Exported: ${entry.name}' : 'Export cancelled'),
            backgroundColor: success
                ? CyberpunkTheme.neonGreen.withOpacity(0.9)
                : CyberpunkTheme.surface,
          ),
  ...
  Future<void> _deleteFolder(String folderPath) async {
    SecureLogger.d(
        'VaultHomeScreen', '_deleteFolder: starting deletion of $folderPath');
    widget.stateManager.recordActivity();

    // Get file count for confirmation message
    final fileCount =
        widget.stateManager.getFileCountInFolderRecursive(folderPath);
    final folderName = folderPath.split('/').last;
    SecureLogger.d('VaultHomeScreen',
        '_deleteFolder: folderName=$folderName, fileCount=$fileCount');
            ...
            if (fileCount > 0)
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  border:
                      Border.all(color: CyberpunkTheme.error.withOpacity(0.3)),
                ),
                child: Row(
                  children: [
                    const Icon(Icons.warning_amber,
                        color: CyberpunkTheme.error, size: 20),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
    ...
    if (confirm == true) {
      SecureLogger.d(
          'VaultHomeScreen', '_deleteFolder: user confirmed deletion');
      // Freeze timers during folder deletion (involves vault write operations)
      widget.stateManager.freezeTimers();
      setState(() => _isVaultOp = true);
      try {
        SecureLogger.d('VaultHomeScreen',
            '_deleteFolder: calling stateManager.deleteFolder');
        final deletedCount = await widget.stateManager.deleteFolder(folderPath);
        SecureLogger.d(
            'VaultHomeScreen', '_deleteFolder: deletedCount=$deletedCount');

        // Refresh files without reloading the folder map we just updated.
        SecureLogger.d(
            'VaultHomeScreen', '_deleteFolder: calling refreshEntriesOnly');
        await widget.stateManager.refreshEntriesOnly();
        SecureLogger.d('VaultHomeScreen',
            '_deleteFolder: deletion completed successfully');

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text('Deleted folder "$folderName" ($deletedCount files)'),
              backgroundColor: CyberpunkTheme.warning,
            ),
          );
        }
    ...
    if (newName != null && newName.isNotEmpty && newName != folderName) {
      ...
      try {
        ...
        if (_currentFolderPath == folderPath) {
          ...
          setState(() {
            _currentFolderPath =
                newFolderPath + _currentFolderPath.substring(folderPath.length);
          });
        }
        ...
        child: Scaffold(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.key,
                            size: 20, color: CyberpunkTheme.neonGreen),
                        const SizedBox(width: 12),
                        const Text('Change Password',
                            style:
                                TextStyle(color: CyberpunkTheme.textPrimary)),
                      ],
                    ),
                  ),
                  PopupMenuItem(
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintStyle:
                          const TextStyle(color: CyberpunkTheme.textHint),
                      prefixIcon: Icon(
                        Icons.search,
                      ...
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12),
                        borderSide:
                            BorderSide(color: CyberpunkTheme.surfaceBorder),
                      ),
                      ...
                      focusedBorder: OutlineInputBorder(
                        ...
                        borderSide: const BorderSide(
                            color: CyberpunkTheme.neonGreen, width: 2),
                      ),
          ...
          floatingActionButton: Container(
            ...
            child: FloatingActionButton(
              onPressed: (_isImporting || _isExporting || _isVaultOp)
                  ? null
                  : _showImportOptions,
              backgroundColor: CyberpunkTheme.neonGreen,
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.file_download_outlined,
                              size: 20, color: CyberpunkTheme.warning),
                          const SizedBox(width: 12),
                          const Text('Export',
                              style: TextStyle(color: CyberpunkTheme.warning)),
                        ],
                      ),
                    ),
                    PopupMenuItem(
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.delete_outline,
                              size: 20, color: CyberpunkTheme.error),
                          const SizedBox(width: 12),
                          const Text('Delete',
                              style: TextStyle(color: CyberpunkTheme.error)),
                        ],
                      ),
                    ),
                  ],
              ...
              if (onRename != null)
                ListTile(
                  ...
                  subtitle: const Text(
                    'Change folder name',
                    style:
                        TextStyle(color: CyberpunkTheme.textHint, fontSize: 12),
                  ),
                  onTap: () {
