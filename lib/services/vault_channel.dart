...
class VaultChannel {
  ...
  static Future<Map<String, dynamic>> importFile(String uri,
      {int? sessionId}) async {
  ...
  static Future<Uint8List> readFile(List<int> fileId) async {
    final result =
        await _channel.invokeMethod<Uint8List>('readFile', {'fileId': fileId});
    return result!;
  }

  /// Read text preview from vault (max 1MB for large files)
  /// Returns map with: data (Uint8List), truncated (bool), totalSize (int)
  static Future<Map<String, dynamic>> readTextPreview(List<int> fileId,
      {int maxBytes = 1024 * 1024}) async {
  ...
  /// Render one PDF page in native memory without writing plaintext PDF to disk.
  static Future<Map<String, dynamic>> renderPdfPage(List<int> fileId,
      {int pageIndex = 0, int width = 1080}) async {
    final result = await _channel.invokeMethod<Map>('renderPdfPage', {
      'fileId': fileId,
      'pageIndex': pageIndex,
      'width': width,
    });
  ...
  static Future<List<int>> copyFile(List<int> fileId) async {
    final result =
        await _channel.invokeMethod<List>('copyFile', {'fileId': fileId});
    return (result ?? <int>[]).cast<int>();
  }
  ...
  static Future<bool> authenticateBiometric({bool throwOnError = false}) async {
    try {
      return await _channel.invokeMethod<bool>('authenticateBiometric') ??
          false;
    } on PlatformException {
      if (throwOnError) {
        rethrow;
  ...
  /// Reset rate limiter only after passphrase + biometric have both succeeded.
  static Future<void> recordAuthSuccess({String? vaultId}) async {
    await _channel.invokeMethod('recordAuthSuccess', {'vaultId': vaultId});
  }
  ...
  static Future<bool> playVideo(int handle) async {
    return await _channel.invokeMethod<bool>('playVideo', {'handle': handle}) ??
        false;
  }
  ...
  static Future<bool> pauseVideo(int handle) async {
    return await _channel
            .invokeMethod<bool>('pauseVideo', {'handle': handle}) ??
        false;
  }
  ...
  static Future<int> getVideoPosition(int handle) async {
    return await _channel
            .invokeMethod<int>('getVideoPosition', {'handle': handle}) ??
        0;
  }
  ...
  static Future<int> getVideoDuration(int handle) async {
    return await _channel
            .invokeMethod<int>('getVideoDuration', {'handle': handle}) ??
        0;
  }
  ...
  static Future<bool> isVideoPlaying(int handle) async {
    return await _channel
            .invokeMethod<bool>('isVideoPlaying', {'handle': handle}) ??
        false;
  }
  ...
  static Future<bool> closeVideo(int handle) async {
    return await _channel
            .invokeMethod<bool>('closeVideo', {'handle': handle}) ??
        false;
  }

  /// Play audio by handle
  static Future<bool> playAudio(int handle) async {
    return await _channel.invokeMethod<bool>('playAudio', {'handle': handle}) ??
        false;
  }
  ...
  static Future<bool> pauseAudio(int handle) async {
    return await _channel
            .invokeMethod<bool>('pauseAudio', {'handle': handle}) ??
        false;
  }
  ...
  static Future<int> getAudioPosition(int handle) async {
    return await _channel
            .invokeMethod<int>('getAudioPosition', {'handle': handle}) ??
        0;
  }
  ...
  static Future<int> getAudioDuration(int handle) async {
    return await _channel
            .invokeMethod<int>('getAudioDuration', {'handle': handle}) ??
        0;
  }
  ...
  static Future<bool> isAudioPlaying(int handle) async {
    return await _channel
            .invokeMethod<bool>('isAudioPlaying', {'handle': handle}) ??
        false;
  }
  ...
  static Future<bool> closeAudio(int handle) async {
    return await _channel
            .invokeMethod<bool>('closeAudio', {'handle': handle}) ??
        false;
  }
  ...
  static Future<Map<String, dynamic>?> importVaultWithTitle(
