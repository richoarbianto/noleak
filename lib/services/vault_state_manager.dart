...
class VaultStateManager extends ChangeNotifier {
  ...
  Future<void> unlockVaultById(String vaultId, {String? password}) async {
    ...
    if (titleEntry != null) {
      try {
        ...
        SecureLogger.d(
            'VaultStateManager', 'Title loaded: $_currentVaultTitle');
      } catch (e) {
    ...
    _startSessionTimer();
    SecureLogger.d(
        'VaultStateManager', 'State set to UNLOCKED, calling notifyListeners');
    notifyListeners();
  }
    ...
    try {
      ...
      final entries = entriesData
          .map((e) => VaultEntry(
                ...
                size: e['size'] as int,
                createdAt:
                    DateTime.fromMillisecondsSinceEpoch(e['createdAt'] as int),
  ...
  Future<void> createFolder(String name, {String parent = ''}) async {
    SecureLogger.d(
        'VaultStateManager', 'createFolder: name=$name, parent=$parent');
    ...
    SecureLogger.d('VaultStateManager',
        'createFolder: normalizedParent=$normalizedParent, normalizedName=$normalizedName');
    if (normalizedName.isEmpty) {
      SecureLogger.e('VaultStateManager',
          'createFolder: folder name is empty after normalization');
    ...
    if (fullPath.isEmpty) {
      SecureLogger.e('VaultStateManager',
          'createFolder: fullPath is empty after normalization');
      throw Exception('Invalid folder name');
    }
    if (_folders.contains(fullPath)) {
      throw Exception('Folder already exists');
    }
    final added = _addFolderWithParents(fullPath);
    SecureLogger.d('VaultStateManager',
        'createFolder: _addFolderWithParents returned $added, folders=${_folders.toList()}');

    // Only persist if folder was actually added (optimization for large vaults)
    if (added) {
      await _persistFolderMap();
      SecureLogger.d('VaultStateManager',
          'createFolder: folder map persisted successfully');
    } else {
      SecureLogger.d('VaultStateManager',
          'createFolder: folder already exists, skipping persist');
    }
    _safeNotify();
  }
  ...
  Future<int> deleteFolder(String folderPath) async {
    SecureLogger.d(
        'VaultStateManager', 'deleteFolder: starting deletion of $folderPath');
    final normalized = _normalizeFolderPath(folderPath);
    if (normalized.isEmpty) {
      SecureLogger.e('VaultStateManager',
          'deleteFolder: invalid folder path (empty after normalization)');
      throw Exception('Invalid folder path');
    }

    if (!_folders.contains(normalized)) {
      SecureLogger.e('VaultStateManager',
          'deleteFolder: folder does not exist: $normalized');
      throw Exception('Folder does not exist');
    }

    SecureLogger.d(
        'VaultStateManager', 'deleteFolder: deleting folder: $normalized');
    ...
    SecureLogger.d('VaultStateManager',
        'deleteFolder: found ${filesToDelete.length} files to delete');

    // Delete all files in the folder
    for (final entry in filesToDelete.entries) {
      try {
        SecureLogger.d('VaultStateManager',
            'deleteFolder: deleting file ${entry.key.substring(0, 8)}...');
        ...
        deletedCount++;
        SecureLogger.d('VaultStateManager',
            'deleteFolder: deleted file ${entry.key.substring(0, 8)}...');
      } catch (e) {
        SecureLogger.e(
            'VaultStateManager',
            'deleteFolder: failed to delete file ${entry.key.substring(0, 8)}...',
            e);
    ...
    final foldersToRemove = _folders
        ...
        .toList();
    SecureLogger.d('VaultStateManager',
        'deleteFolder: removing ${foldersToRemove.length} folders: $foldersToRemove');
    ...
    SecureLogger.d('VaultStateManager',
        'deleteFolder: completed, deleted $deletedCount files');
    return deletedCount;
  }
  ...
  Future<void> renameFolder(String oldPath, String newName) async {
    ...
    final parentPath =
        lastSlash > 0 ? normalizedOld.substring(0, lastSlash) : '';
    ...
    SecureLogger.d('VaultStateManager',
        'Renaming folder: $normalizedOld -> $normalizedNew');
    ...
    for (final folder in foldersToUpdate) {
      ...
      if (folder == normalizedOld) {
        ...
        final newSubPath =
            normalizedNew + folder.substring(normalizedOld.length);
    ...
    for (final entry in _fileFolders.entries) {
      ...
      if (fileFolder == normalizedOld) {
        ...
        final newFileFolder =
            normalizedNew + fileFolder.substring(normalizedOld.length);
    ...
    SecureLogger.d('VaultStateManager',
        'Folder renamed: $normalizedOld -> $normalizedNew');
  }
  ...
  Future<void> applyImportedFiles(Map<List<int>, String> fileFolders) async {
    SecureLogger.d('VaultStateManager',
        'applyImportedFiles: processing ${fileFolders.length} files');
    ...
    for (final entry in fileFolders.entries) {
      ...
      SecureLogger.d('VaultStateManager',
          'applyImportedFiles: fileId=${_fileIdToHex(entry.key).substring(0, 8)}..., folder=$folder');
      if (folder.isNotEmpty) {
        final added = _addFolderWithParents(folder);
        SecureLogger.d('VaultStateManager',
            'applyImportedFiles: _addFolderWithParents($folder) returned $added');
      }
      _fileFolders[_fileIdToHex(entry.key)] = folder;
    }
    SecureLogger.d('VaultStateManager',
        'applyImportedFiles: folders after update=${_folders.toList()}');

    await _persistFolderMap();
    SecureLogger.d('VaultStateManager',
        'applyImportedFiles: folder map persisted successfully');
  ...
  Future<void> removeFileFromFolderMap(List<int> fileId) async {
    ...
    SecureLogger.d('VaultStateManager',
        'removeFileFromFolderMap: removing file ${hexId.substring(0, 8)}...');
    final removed = _fileFolders.remove(hexId) != null;
    SecureLogger.d(
        'VaultStateManager', 'removeFileFromFolderMap: removed=$removed');
    if (removed) {
      await _persistFolderMap();
      SecureLogger.d(
          'VaultStateManager', 'removeFileFromFolderMap: folder map persisted');
      notifyListeners();
    }
  }
  ...
  Future<void> _loadFolderMap(List<VaultEntry> allEntries) async {
    SecureLogger.d('VaultStateManager',
        '_loadFolderMap: starting, allEntries=${allEntries.length}');
    _fileFolders.clear();
    _folders.clear();

    final candidates = allEntries
        .where((e) => e.name == _folderMapName || e.name == _folderMapTempName)
        .toList()
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt));

    if (candidates.isEmpty) {
      SecureLogger.d('VaultStateManager',
          '_loadFolderMap: no mapEntry found, returning empty');
      return;
    }
    ...
    for (final candidate in candidates) {
      Uint8List? mapBytes;
      try {
        mapBytes = await VaultChannel.readFile(candidate.fileId);
        SecureLogger.d('VaultStateManager',
            '_loadFolderMap: read ${mapBytes.length} bytes from ${candidate.name}');
        final decoded =
            jsonDecode(utf8.decode(mapBytes)) as Map<String, dynamic>;
        SecureLogger.d('VaultStateManager',
            '_loadFolderMap: decoded version=${decoded['version']}');
        if (decoded['version'] != _folderMapVersion) {
          SecureLogger.w('VaultStateManager',
              '_loadFolderMap: version mismatch, expected $_folderMapVersion');
          continue;
        }
        ...
        if (folders is List) {
          SecureLogger.d('VaultStateManager',
              '_loadFolderMap: loading ${folders.length} folders');
        ...
        if (files is Map) {
          SecureLogger.d('VaultStateManager',
              '_loadFolderMap: loading ${files.length} file mappings');
        ...
        SecureLogger.d('VaultStateManager',
            '_loadFolderMap: loaded folders=${_folders.toList()}');
        loaded = true;
        break;
      } catch (e) {
        SecureLogger.e('VaultStateManager',
            '_loadFolderMap: candidate failed (${candidate.name})', e);
      } finally {
    ...
    final existing = allEntries
        ...
        .toSet();
    final removedKeys =
        _fileFolders.keys.where((k) => !existing.contains(k)).toList();
  ...
  Future<void> _persistFolderMap() async {
    SecureLogger.d('VaultStateManager',
        '_persistFolderMap: starting, folders=${_folders.toList()}, files=${_fileFolders.length}');
    ...
    SecureLogger.d(
        'VaultStateManager', '_persistFolderMap: payload size=${bytes.length}');
    try {
      ...
      for (int attempt = 0; attempt < maxRetries; attempt++) {
        try {
          SecureLogger.d(
              'VaultStateManager', '_persistFolderMap: attempt ${attempt + 1}');
          await _upsertSystemFile(_folderMapName, bytes,
              mime: 'application/json');
          SecureLogger.d('VaultStateManager',
              '_persistFolderMap: _upsertSystemFile completed');
          return; // Success
        } catch (e) {
          SecureLogger.w('VaultStateManager',
              'Folder map persist attempt ${attempt + 1} failed: $e');
  ...
  Future<void> _upsertSystemFile(String name, Uint8List data,
      {String? mime}) async {
    SecureLogger.d('VaultStateManager',
    ...
    SecureLogger.d('VaultStateManager',
        '_upsertSystemFile: system file appended, fileId=${created.take(4)}');

    final createdHex = _fileIdToHex(created);
    final saved = (await VaultChannel.listFiles()).any((entry) {
      if (entry['name'] != name) return false;
      return _fileIdToHex(List<int>.from(entry['fileId'] as List)) ==
          createdHex;
    });
    if (!saved) {
