...
class _NoLeakAppState extends State<NoLeakApp> with WidgetsBindingObserver {
  ...
  bool _clearingVaultRoutes = false;

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addObserver(this);
    _stateManager.addListener(_handleVaultStateChange);
    _initialize();
  }
  ...
  void dispose() {
    ...
    _stateManager.removeListener(_handleVaultStateChange);
    _stateManager.dispose();
    _registry.dispose();
    super.dispose();
  }

  void _handleVaultStateChange() {
    if (_stateManager.state == VaultState.unlocked || _clearingVaultRoutes) {
      return;
    }
    _clearingVaultRoutes = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final navigator = _navigatorKey.currentState;
      if (navigator != null) {
        navigator.popUntil((route) => route.isFirst);
      }
      if (mounted && _selectedVaultId != null) {
        setState(() => _selectedVaultId = null);
      }
      _clearingVaultRoutes = false;
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    switch (state) {
      ...
      case AppLifecycleState.detached:
        SecureLogger.d(
            'NoLeakApp', 'App backgrounded ($state) - triggering lock');
  ...
  Widget _buildHome() {
    ...
    if (_selectedVaultId != null) {
      SecureLogger.d(
          'NoLeakApp', 'Building with selectedVaultId: $_selectedVaultId');
      return ListenableBuilder(
        listenable: _stateManager,
        builder: (context, _) {
          SecureLogger.d('NoLeakApp',
              'ListenableBuilder rebuild, state: ${_stateManager.state}');
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              _error!,
                              style:
                                  const TextStyle(color: CyberpunkTheme.error),
                            ),
                          ),
                        ],
                      ),
